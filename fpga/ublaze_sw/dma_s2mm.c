...
int Dma_ResetCore(void) {
  Xil_Out32(AXI_DMA_BASEADDR + AXI_DMA_MM2S_DMACR_OFFSET, AXI_DMA_CR_RESET_MASK);

  for (u32 i = 0; i < 1000000; i++) {
    if ((Xil_In32(AXI_DMA_BASEADDR + AXI_DMA_MM2S_DMACR_OFFSET) & AXI_DMA_CR_RESET_MASK) == 0)
      return 1;
  }
  return 0;
}
...
void DmaS2mm_ArmTransfer(u32 dest_addr, u32 length_bytes) {
  ...
  Xil_Out32(AXI_DMA_BASEADDR + AXI_DMA_S2MM_DA_OFFSET, dest_addr);
  ...
  cr = Xil_In32(AXI_DMA_BASEADDR + AXI_DMA_S2MM_DMACR_OFFSET);
  Xil_Out32(AXI_DMA_BASEADDR + AXI_DMA_S2MM_DMACR_OFFSET,
  ...
  Xil_Out32(AXI_DMA_BASEADDR + AXI_DMA_S2MM_LENGTH_OFFSET, length_bytes);
}

void DmaMm2s_ArmTransfer(u32 src_addr, u32 length_bytes) {
  u32 cr;

  Xil_Out32(AXI_DMA_BASEADDR + AXI_DMA_MM2S_SA_OFFSET, src_addr);

  cr = Xil_In32(AXI_DMA_BASEADDR + AXI_DMA_MM2S_DMACR_OFFSET);
  Xil_Out32(AXI_DMA_BASEADDR + AXI_DMA_MM2S_DMACR_OFFSET, cr | AXI_DMA_CR_RUNSTOP_MASK);

  Xil_Out32(AXI_DMA_BASEADDR + AXI_DMA_MM2S_LENGTH_OFFSET, length_bytes);
}

DmaS2mmResult DmaS2mm_PollComplete(u32 max_iters) {
  for (u32 i = 0; i < max_iters; i++) {
    u32 sr = Xil_In32(AXI_DMA_BASEADDR + AXI_DMA_S2MM_DMASR_OFFSET);
...
void DmaS2mm_AckComplete(void) {
  Xil_Out32(AXI_DMA_BASEADDR + AXI_DMA_S2MM_DMASR_OFFSET,
