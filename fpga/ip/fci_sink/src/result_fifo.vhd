...
--
-- Deliberately a byte-for-byte sibling of psd_core/src/result_fifo.vhd rather than a shared file:
-- these are independently packaged IP cores and must not depend on a common library, or adding one
-- to a block design would drag the other's sources in with it. The duplication is the price of
-- that independence, and the two are meant to stay identical.
--
...
-- stalled (see fci_sink_top's header), so backpressuring on a full FIFO is not available; losing
