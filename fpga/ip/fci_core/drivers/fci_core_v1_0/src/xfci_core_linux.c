...
typedef struct {
    ...
    XFci_core_uio_map maps[ MAX_UIO_MAPS ];
} XFci_core_uio_info;

/***************** Variable Definitions **************************************/
static XFci_core_uio_info uio_info;
...
static int uio_info_read_name(XFci_core_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XFci_core_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XFci_core_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XFci_core_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XFci_core_Initialize(XFci_core *InstancePtr, const char* InstanceName) {
	XFci_core_uio_info *InfoPtr = &uio_info;
