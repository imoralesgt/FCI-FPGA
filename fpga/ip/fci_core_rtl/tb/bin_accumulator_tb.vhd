...
architecture sim of bin_accumulator_tb is
  ...
  constant NFFT       : integer := 11; -- log2(FFT_LENGTH), stated independently of the DUT
...
begin
  ...
  clk_i <= not clk_i after CLK_PERIOD / 2 when sim_done = '0' else '0';
  ...
  begin
    ...
    set_windows(0, 0, 0, NBINS - 1);
    drive_frame(false);
    got_l := to_integer(unsigned(psa_l_o));
    got_w := to_integer(unsigned(psa_w_o));
    check("window [0,0] captures DC (got " & integer'image(got_l) & ", expected "
          & integer'image(mag_of_bin(0)) & ")", got_l = mag_of_bin(0));
    check("window [0," & integer'image(NBINS - 1) & "] captures the whole spectrum (got "
          & integer'image(got_w) & ", expected "
          & integer'image(expected_sum(0, NBINS - 1)) & ")",
          got_w = expected_sum(0, NBINS - 1));
